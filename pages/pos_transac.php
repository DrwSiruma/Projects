...
session_start();
                ...
                for($i=1; $i<=$countID; $i++)
                  {
                    ...
                    $query = "INSERT INTO `transaction_details`
                               (`ID`, `TRANS_D_ID`, `PRODUCTS`, `QTY`, `PRICE`, `EMPLOYEE`, `ROLE`, `transac_date`)
                               VALUES (Null, '{$today}', '".$_POST['name'][$i-1]."', '".$_POST['quantity'][$i-1]."', '".$_POST['price'][$i-1]."', '{$emp}', '{$rol}', '{$date}')";

                    mysqli_query($db,$query)or die (mysqli_error($db));

                    $pqty = $_POST['quantity'][$i-1]  ;
                    $pname = $_POST['name'][$i-1];

                    $qtyqry = "UPDATE `product` SET `QTY_STOCK` = (`QTY_STOCK` - '$pqty'), `ON_HAND` = (`ON_HAND` - '$pqty') WHERE NAME = '$pname'";
                    mysqli_query($db,$qtyqry)or die (mysqli_error($db));

                  }

                    $query111 = "INSERT INTO `transaction`
                               (`TRANS_ID`, `CUST_ID`, `NUMOFITEMS`, `SUBTOTAL`, `GRANDTOTAL`, `CASH`, `DATE`, `TRANS_D_ID`)
                               VALUES (Null,'{$customer}','{$countID}','{$subtotal}','{$total}','{$cash}','{$date}','{$today}')";
              ...
              <script type="text/javascript">
                alert("Success.");
                window.location = "pos_receipt.php?id=<?php echo $today; ?>";
              </script>
            <?php } else {?>
              <script type="text/javascript">
